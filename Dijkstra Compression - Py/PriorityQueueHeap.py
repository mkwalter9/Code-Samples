...
class PriorityQueueHeap:
    ...
    def decreaseKey(self, vertex, newValue):
        ...
        while True:
            ind  = self.index[vertex.value]
            parent = self.array[ind//2]
            leftc = self.array[2*ind]
            rightc = self.array[2*ind + 1]
            if leftc < vertex:
                swap(vertex, leftc)
            elif rightc < vertex:
                swap(vertex, rightc)
    ...
    def swap(self, index1, index2):
        ...
        vertex1 = self.array[index1]
        vertex2 = self.array[index2]
        self.array[index2] = vertex1
        self.index[vertex1] = index2
        self.array[index1] = vertex2
